...
export interface IStorage {
  ...
  getInventoryAlertsByProduct(productName: string, productVariant?: string): Promise<InventoryAlert[]>;
  getActiveInventoryAlerts(): Promise<InventoryAlert[]>;
  updateInventoryAlert(id: number, updates: Partial<InventoryAlert>): Promise<InventoryAlert>;
  deactivateInventoryAlert(id: number): Promise<void>;

  // Product Stock
  getProductStock(productName: string, productVariant?: string): Promise<ProductStock | undefined>;
  createProductStock(stock: InsertProductStock): Promise<ProductStock>;
  updateProductStock(productName: string, productVariant: string | null, updates: Partial<ProductStock>): Promise<ProductStock>;
  getLowStockProducts(): Promise<ProductStock[]>;

  // Inventory Alert Log
  createInventoryAlertLog(log: InsertInventoryAlertLog): Promise<InventoryAlertLog>;
  getInventoryAlertLogs(alertId: number): Promise<InventoryAlertLog[]>;
}
    ...
    return db.select()
      ...
      .where(
        and(
          ...
          // Use a raw SQL expression to compare stockLevel with lowStockThreshold
