...
${chatSummary}
  ...
  app.post("/api/user/preferences", async (req: Request, res: Response) => {
    try {
      const { userId, language, name, email, preferences } = req.body;
      ...
      if (existingPreferences) {
        ...
        result = await storage.updateUserPreferences(userId, {
          language,
          name,
          email,
          preferences,
        ...
        result = await storage.createUserPreferences({
          userId,
          language,
          name,
          email,
  ...
  app.patch("/api/user/preferences/:userId/language", async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
      ...
      if (!['nl', 'en', 'fr', 'tr'].includes(language)) {
        return res.status(400).json({ message: "Invalid language. Must be one of: nl, en, fr, tr" });
      }
      ...
      let result;
      if (existingPreferences) {
        result = await storage.updateUserLanguage(userId, language);
      } else {
        result = await storage.createUserPreferences({
          userId,
          language,
