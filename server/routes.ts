...
${chatSummary}
  ...
  // Fly Screen Order Routes
  app.post("/api/fly-screen-orders", formRateLimiter, spamDetectionMiddleware, async (req: Request, res: Response) => {
    try {
      const validatedData = insertFlyScreenOrderSchema.parse(req.body);
      
      const order = await flyScreenStorage.createFlyScreenOrder(validatedData);
      
      // Send confirmation emails
      const customerEmailHtml = `
        <h2>Bedankt voor uw bestelling!</h2>
        <p>Beste ${order.firstName} ${order.lastName},</p>
        <p>Wij hebben uw bestelling voor een vliegengordijn ontvangen.</p>
        
        <h3>Bestelling Details:</h3>
        <ul>
          <li><strong>Bestelnummer:</strong> ${order.orderNumber}</li>
          <li><strong>Afmetingen:</strong> ${order.width}cm x ${order.height}cm</li>
          <li><strong>Kaderkleur:</strong> ${order.frameColor}</li>
          <li><strong>Gaaskleur:</strong> ${order.meshColor}</li>
          <li><strong>Totaalprijs:</strong> €${order.totalPrice.toFixed(2)}</li>
        </ul>
        
        <h3>Leveradres:</h3>
        <p>${order.deliveryAddress}<br>
        ${order.deliveryZipCode} ${order.deliveryCity}</p>
        
        <p>Productietijd: 7-10 werkdagen na bevestiging betaling</p>
        <p>U ontvangt binnenkort een betalingslink.</p>
        
        <p>Met vriendelijke groet,<br>Team KANIOU</p>
      `;
      
      const adminEmailHtml = `
        <h2>Nieuwe Vliegengordijn Bestelling</h2>
        <p>Er is een nieuwe bestelling binnengekomen:</p>
        
        <h3>Klantgegevens:</h3>
        <ul>
          <li><strong>Naam:</strong> ${order.firstName} ${order.lastName}</li>
          <li><strong>Email:</strong> ${order.email}</li>
          <li><strong>Telefoon:</strong> ${order.phone || 'Niet opgegeven'}</li>
        </ul>
        ...
        <ul>
          ...
          <li><strong>Gaaskleur:</strong> ${order.meshColor}</li>
          <li><strong>Totaalprijs:</strong> €${order.totalPrice.toFixed(2)}</li>
          <li><strong>Opmerkingen:</strong> ${order.notes || 'Geen'}</li>
        </ul>
      ...
      console.error("Fly screen order error:", error);
      ...
      res.status(500).json({
        success: false,
        message: "Er is een fout opgetreden bij het verwerken van uw bestelling"
      });
    }
  });

  // Calculate fly screen price
  app.post("/api/fly-screen-price", async (req: Request, res: Response) => {
    try {
      const { width, height, meshColor } = req.body;
      
      if (!width || !height) {
        return res.status(400).json({
          success: false,
          message: "Breedte en hoogte zijn verplicht"
        });
      }
      ...
      if (width > 100) {
        totalPrice += Math.ceil((width - 100) / 10) * 10;
      }
      if (height > 100) {
        totalPrice += Math.ceil((height - 100) / 10) * 10;
      }
      ...
      res.json({
        ...
        breakdown: {
          base: basePrice,
          extraWidth: width > 100 ? Math.ceil((width - 100) / 10) * 10 : 0,
          extraHeight: height > 100 ? Math.ceil((height - 100) / 10) * 10 : 0,
          blackMesh: meshColor === 'black' ? 5 : 0
        }
      });
    } catch (error) {
      ...
      res.status(500).json({
        success: false,
        message: "Fout bij prijsberekening"
      });
    }
  });
